PROGRAM RafserviceSchedule;
   USES
      CRT, SYSUTILS,DOS;
         TYPE
           ...
           StaffRec=RECORD
            ...
            AircraftJobRec=RECORD
                               AircraftjobId       :  LONGINT;
                               JobID               :  STRING[5];
                               AircraftID          :  STRING[3];
                               complete            :  BOOLEAN;
                            END;
            ...
            AircraftFiletype                  = FILE OF AircraftRec;
     ...
     PROCEDURE Delay(Year,Month,Day,Number:LONGINT);
        VAR
           Subtract,JobDay,Today,IntialRow,IntialCol,TodayYear,TodayMonth,TodayDay,TodayNo,Time,AircraftPostion,CurrentJobId,Add:LONGINT;
           ...
           BEGIN
              ...
              BEGIN
                 ...
                 BEGIN
                    ...
                    RESET(AircraftJobFile);
                    REPEAT
                       READ(AircraftJobFile,JetJob);
                       IF (Jetjob.AircraftJObID=CurrentJobId) AND (Jetjob.Complete=FALSE) THEN
                       Found:=TRUE;
                    UNTIL (Found) OR (EOF(AircraftJobFile));
                    IF Found THEN
                    BEGIN
                       AircraftPostion:=FILEPOS(AircraftJobFile)-1;
                       BEGIN
                          RESET(AircraftFile);
                          REPEAT
                             READ(AircraftFile,Jet);
                          UNTIL Jet.AircraftId=JetJob.AircraftId;
                       END;
                       BEGIN
                          RESET(JobFile);
                          REPEAT
                       ...
                       BEGIN
                          ...
                          BEGIN
                             ...
                             WRITELN('Aircraft Id: ',AircraftId);
                             WRITELN('Call sign: ',CallSign);
                             WRITELN('Status: ',AircraftStatus);
                          ...
                          IF Finished='Y' THEN
                          BEGIN
                             SEEK(AircraftJobFile,AircraftPostion);
                             JetJob.Complete:=TRUE;
                             WRITE(AircraftJobFile,JetJob);
                             SEEK(AIRCRAFTJOBFILE,AIRCRAFTPOSTION);
                             READ(AircraftJobFile,jetjob);
                          END;
                          IF Finished='N' THEN
                 ...
                 BEGIN
                    ...
                    BEGIN
                       ...
                       BEGIN
                          ...
                          REPEAT
                             READ(AircraftJobFile,JetJob);
                          UNTIL Jetjob.AircraftJobId=TaskID;
                          MatchId:=JetJob.JobId;
                          Plane:=Jetjob.AircraftId;
                          RESET(JobFile);
                          REPEAT
                             READ(JobFile,Job);
                          ...
                          BEGIN
                             ...
                             BEGIN
                                ...
                                REPEAT
                                    READ(AircraftFile,Jet);
                                UNTIL(Jet.Aircraftid=Plane) OR (EOF(AircraftFile));
                                WITH Jet DO
                                BEGIN
                                   WRITELN('Aircraft');
                                   WRITELN;
    ...
    PROCEDURE ViewFile;
       ...
       BEGIN
          REPEAT
             ...
             WRITELN('Press B to view the aircraft file');
          //   WRITELN('Press C to view the calander file');
             ...
             CASE Filechoice OF
               ...
               'B':BEGIN
                      RESET(AircraftFile);
                      WHILE NOT EOF(AircraftFile) DO
                      BEGIN
                         READ(AircraftFile,Jet);
                         WITH JET DO
                         BEGIN
                            IF AircraftId <> '-1' THEN
                            BEGIN
                               WRITELN;
                               WRITELN('Aircraft ID: ',AircraftId);
                               WRITELN('Call sign: ',CallSign);
                               WRITELN('Aircraft Status: ',AircraftStatus);
                               WRITELN('Flight hours since last service: ',Flight_timefromservice);
   ...
   PROCEDURE Search(SearchItem:CHAR);
      ...
      BEGIN
         ...
         REPEAT
            ...
            BEGIN
               ...
               WRITELN('Press B to search for a aircraft');
               WRITELN('Press x to exit');
            ...
            CASE SearchItem OF
               ...
               'B':BEGIN
                      REPEAT
                         ...
                         RESET(AircraftFile);
                         REPEAT
                            WRITELN('Do you wish to view the list of current aircraft (Y=Yes,N=No)');
                         ...
                         END;
                         WRITELN;
                         WRITELN('Please enter the Id number of the desired aircraft');
                         READLN(SearchName);
                         Located:=FALSE;
                         RESET(AircraftFile);
                         BEGIN
                            REPEAT
                               READ(AircraftFile,Jet);
                               IF UPPERCASE(Jet.AircraftId) = UPPERCASE(SearchName) THEN
                               Located:=TRUE;
                            UNTIL (Located) OR (EOF(AircraftFile));
    ...
    {FUNCTION AssignPriority (CurrentJobCode,NewJobCode:STRING;AircraftPos:LONGINT):LONGINT;
       BEGIN
          RESET(JobFile);
          REPEAT
             READ(JobFile,Job);
          UNTIL Job.JobID=NewJobCode;
          SEEK(AircraftFile,AircraftPos);
          READ(AircraftFile,Jet);
          BEGIN
             RESET(JobFile);
             REPEAT
          ...
          WITH Jet DO
          BEGIN
             IF AircraftStatus='Job in progress' THEN}
...
//READKEY;
   ...
   PROCEDURE AddAuths;
      ...
      BEGIN
         REPEAT
            ...
            WRITELN('Press A to assing  Aircraft jack');
   ...
   PROCEDURE CollectData;
      ...
      BEGIN
         ...
         WRITELN('Press B to appened the Aircraft file');
         ...
         CASE Choice Of
            ...
            'B':BEGIN
                   ...
                   BEGIN
                      REPEAT
                         WRITELN('Pleasse enter the provided aircraft Id');
                         READLN(AircraftId);
                         IF LENGTH(AircraftId)<>4  THEN
                         BEGIN
                      ...
                      REPEAT
                         CLRSCR;
                         WRITELN('Please assign the suitable aircraft status');
                         WRITELN;
                         WRITELN('Please select one of the following options');
                         WRITELN('1:Serviable');
                         ...
                         Option:=UPCASE(READKEY);
                         CASE Option OF
                            '1':AircraftStatus:='=Servicable';
                            '2':Aircraftstatus:='Job in progress';
                            '3':Aircraftstatus:='Unservicable';
                            ELSE
                            BEGIN
                               WRITELN('Invalid input please try again');
                               WRITELN;
            ...
            'C':BEGIN
                ...
                END;
            'D':BEGIN
                   WRITELN('Please select the aircraft that needs servecing');
                   WRITELN;
                   WRITELN('Press anykey to continue');
                   Find:='B';
                   Search(Find);
                   JetId:=Jet.AircraftId;
                   REPEAT
                      CLRSCR;
                      ...
                      BEGIN
                         ...
                         BEGIN
                            ...
                            BEGIN
                               ...
                               REPEAT
                                  READ(AircraftJobFile,JetJob);
                               UNTIL(JetJob.AircraftJobId=-1) OR (EOF(AircraftJobFile));
                               WITH JetJob DO
                               BEGIN
                                  AircraftJobId:=NewTaskId;
                                  JobId:=job.JobId;
                                  AircraftId:=JetId;
                                  Complete:=FALSE;
                                  WRITE(AircraftJobFile,JetJob);
         ...
         REPEAT
            ...
            CASE Choice OF
               ...
               'B':BEGIN
                      ...
                      REPEAT
                         READ(AircraftFile,Jet);
                      UNTIL (Jet.AIrcraftId ='-1') OR (EOF(AIrcraftFile));
                      IF Jet.AircraftId='-1' THEN
                      SEEK(AircraftFile, FILEPOS(AircraftFile)-1);
                      WRITE(AircraftFile, NewRecJet);
              ...
              //  READKEY;
                ...
                BEGIN
                   ...
                   BEGIN
                      ...
                      REPEAT
                         READ(AircraftJobFile,JetJob);
                      UNTIL(AircraftJobId=-1) OR (EOF(AircraftJobFile));
                      AircraftJobId:=NewTask;
                      AircraftId:=JetId;
                      JobID:=Job.JObID;
                      Complete:=FALSE;
                      WRITE(AircraftJobFile,JetJob);
      ...
      BEGIN
         ...
         REPEAT
            ...
            BEGIN
               ...
               WRITELN('Press C to amend aircraft information');
                            ...
                            REPEAT
                               ...
                               BEGIN
                                  REPEAT
                                     ...
                                     WRITELN('Press 2 to amend the status of the aircraft');
                                     WRITELN('Press E to exit');
                                     Options:=UPCASE(READKEY);
                                     CASE Options OF
                                        '1':BEGIN
                                        ...
                                        '2':BEGIN
                                               REPEAT
                                                  CLRSCR;
                                                  WRITELN('Please select the status of the aircraft');
                                                  ...
                                                  Selection:=UPCASE(READKEY);
                                                  CASE Selection OF
                                                     '4':Aircraftstatus:='Servicable';
                                                     '5':Aircraftstatus:='Unservicable';
                                                     '6':BEGIN
                                                            Aircraftstatus:='Job in progress';
                                                            Flight_timefromservice:=0;
                                                         END;
                                                     '7':Aircraftstatus:='Airborn';
                                                     ELSE
                                                     BEGIN
                                                        WRITELN;
                                                        WRITELN('Invalid input please try agian');
   ...
   PROCEDURE Recovrey;
      ...
      BEGIN
         ...
         BEGIN
            ...
            UNTIL EOF(Jobbackup);
            CLOSE(Jobfile);
            REWRITE(Aircraftfile);
            RESET(Aircraftbackup);
            REPEAT
                READ(AircraftBackup,Jet);
                WRITE(AircraftFile,Jet);
            UNTIL EOF(AircraftBackup);
            CLOSE(AircraftFile);
            REWRITE(AuthFile);
            RESET(AuthBackup);
            REPEAT
   ...
   PROCEDURE Backup;
      ...
      BEGIN
         ...
         BEGIN
            ...
            UNTIL EOF(JobFile);
            CLOSE(JobBackup);
            REWRITE(AircraftBackup);
            RESET(AircraftFile);
            REPEAT
                READ(AircraftFile,Jet);
                WRITE(AircraftBackup,Jet);
            UNTIL EOF(AircraftFile);
            CLOSE(AircraftBackup);
            REWRITE(AuthBackup);
            RESET(AuthFile);
            REPEAT
               READ(AuthFile,Auth);
   ...
   PROCEDURE AdminMenu;
      BEGIN
         REPEAT
            ...
            SEEK(StaffFile,StaffRecNo);
            READ(StaffFile,Staff);
            WRITELN('                Welcome to RAf losimouth servecing schedule');
            WRITELN;
   ...
   BEGIN
      ...
      ASSIGN(AircraftFile,    Drive+':\Adam sixth form\Cg4  staff schedule\Aircraft.dta');
      ...
      ASSIGN(AircraftJobFile, Drive+':\Adam sixth form\Cg4  staff schedule\AircraftJob.dta');
      ...
      ASSIGN(AircraftBackup,  Drive+':\Adam sixth form\Cg4  Backup files\AircraftBackup.dta');
